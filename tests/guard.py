...
class CoherenceGuard:
    ...
    def scan_abstract(self, text):
        ...
        # f-Phase: Integration & Validation
        tau = validate.calculate_temporal_tau(text)
        
        # e-Phase: Risk Evaluation
        sigma = zero_return.calculate_risk_sigma(text)
        
        # CORRECTED: Invariant Preservation`n        defensive_terms = ["defense", "safety", "mitigation", "ethical"]`n        if any(term in text.lower() for term in defensive_terms) and sigma < 0.6:`n            verdict = "MODERATE"
        if not boundary_safe:
            sigma = max(sigma, 0.8)  # ESCALATE risk if boundary violated
        
        # CORRECTED: Verdict logic
        if sigma >= 0.6 and risk_hits >= 5 or "offensive" in text.lower():
            verdict = "CRITICAL"
        elif sigma >= 0.45:
            verdict = "HIGH"  
        elif sigma >= 0.30:
